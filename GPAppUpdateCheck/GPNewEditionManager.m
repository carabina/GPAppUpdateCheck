...
#import "GPAppStoreInfoModel.h"

@interface GPNewEditionManager ()
//本地info文件
@property (strong,nonatomic)NSDictionary *infoDict;
@end

@implementation GPNewEditionManager
...
+ (instancetype)shareManager {
    static GPNewEditionManager *instance;
...
+ (void)checkNewEditionWithAppID:(NSString *)appID ctrl:(UIViewController *)containCtrl updateType:(NSInteger)updateType{
    ...
    [[self shareManager] checkNewVersion:appID ctrl:containCtrl updateType:updateType];
    
}

+(void)checkNewEditionWithAppID:(NSString *)appID CustomAlert:(CheckVersionBlock)checkVersionBlock {
    [[self shareManager] getAppStoreVersion:appID sucess:^(GPAppStoreInfoModel *model) {
...
#pragma mark - 立即升级
- (void)updateRightNow:(GPAppStoreInfoModel *)model {
    if ([[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:model.trackViewUrl]]) {
        if (@available(iOS 10.0, *)) {
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:model.trackViewUrl] options:@{} completionHandler:^(BOOL success) {
                exit(0);
            }];
        } else {
...
- (void)getAppStoreVersion:(NSString *)appID sucess:(void(^)(GPAppStoreInfoModel *))update {
    ...
    [self getAppStoreInfo:appID success:^(NSDictionary *respDict) {
        ...
        if (resultCount == 1) {
            ...
            //字典转模型
            GPAppStoreInfoModel *model = [[GPAppStoreInfoModel alloc] init];
